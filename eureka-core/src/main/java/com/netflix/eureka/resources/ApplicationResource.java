...
public class ApplicationResource {
     ...
     * {@link com.netflix.discovery.shared.Application}.
     * 特定实例的注册信息{@link com.netflix.discovery.shared.Application}
     *
     * @param info
    ...
    public Response addInstance(InstanceInfo info,
        ...
        if (isBlank(info.getId())) {
            return Response.status(400).entity("缺少实例Id").build();
        } else if (isBlank(info.getHostName())) {
            return Response.status(400).entity("缺少主机名").build();
        } else if (isBlank(info.getIPAddr())) {
            return Response.status(400).entity("缺少IP地址").build();
        } else if (isBlank(info.getAppName())) {
            return Response.status(400).entity("缺少应用名").build();
        } else if (!appName.equals(info.getAppName())) {
            return Response.status(400).entity("应用名不匹配, 应该是 " + appName + " , 但是是 " + info.getAppName()).build();
        } else if (info.getDataCenterInfo() == null) {
            return Response.status(400).entity("缺少dataCenterInfo").build();
        } else if (info.getDataCenterInfo().getName() == null) {
            return Response.status(400).entity("缺少dataCenterInfo名字").build();
        }
